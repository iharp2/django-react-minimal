...
def gen_file_name(variable: str, datetime: str) -> str:
    ...
    return f"/Users/yuchuanhuang/Github/django-react-minimal/data/{variable}-{datetime}.nc"
...
def raster_data_access_multiple_files(
    ...
    # datetime remove UTC offset, if any
    start_datetime = pd.to_datetime(start_datetime).tz_localize(None).strftime("%Y-%m-%d %H:%M:%S")
    end_datetime = pd.to_datetime(end_datetime).tz_localize(None).strftime("%Y-%m-%d %H:%M:%S")
...
def raster_data_access_files_and_api(
    variable: str,
    min_lat: float,
    max_lat: float,
    min_lon: float,
...
def raster_data_access_pre_aggregation(
    variable: str,
    min_lat: float,
    max_lat: float,
    min_lon: float,
...
def raster_data_access_pre_aggregation_and_api(
    variable: str,
    min_lat: float,
    max_lat: float,
    min_lon: float,
